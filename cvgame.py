...
Matrix = np.zeros([Size, Size])  # 初始化矩阵4*4的0矩阵
...
class NoAction(UpdateNew):
    """docstring for NoAction"""

    def __init__(self,matrix):
        super(NoAction, self).__init__(matrix)
...
def init_game():
    ...
    return matrix,currentscore,screen


# def game(a,matrix,currentscore,screen):
#     for event in pygame.event.get():
#         if event.type == pygame.QUIT:
#             pygame.quit()
#             sys.exit(0)
#     actionObject = GameInit.keyDownPressed(int(a), matrix)  # 创建各种动作类的对象
